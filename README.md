...
| 2023.12.22 | ウリドキ株式会社 入社（在籍中） | CtoBコマース開発部 / Webエンジニア |

## プロジェクト経歴

###### 2023.12~

### リユース品の査定士と出品者の買取マッチングサービスを開発（Ruby on Rails）

###### 課題と解決のための行動

* プロダクトの開発を責任を持ってロードマップやビジョンを抱えている人がいないと気づき、プロダクトマネージャーの役割を担いつつ、UI/UXを改善し、ユーザビリティを向上させている。
* スクラムで開発をしているが、バラバラなまま議論をしても絶対に意味がないため、スクラムマスターを一任。
